...
- 전처리
  ...
  - 불필요/대체 가능 Feature 삭제 및 변경 처리

    - 예측과 관련 없는 Feature (실거래가 아이디, 업무 구분, 물건번호 등) 삭제
    - 명칭을 대체 가능 Feature (지번 코드, 시군구 코드, … 관리 구분코드, 건물 주용도 코드)를 기존 명칭과 동일하게 변경
    - 대지권 면적 Feature 삭제 (아파트는 대지권 면적이 없음)
    
  - 2021년도 실거래 데이터 매칭

    - 기존 Feature를 2016~2021 실거래 데이터에 맞게 가공 (시군구, 거래금액 등)
  - Feature 구성이 기존과 다르거나 매칭이 힘든 부분은 삭제 (번지, 본번, 거래 유형, … 도로명, 중개사 소재지 등)
  
  - 서울시 열린 데이터 광장(2016~2020) 데이터, 국토부 실거래가 공개 시스템(2021) 데이터 병합

      

- 데이터 분석 및 모델 최적화

  - 전처리 완료 데이터를 Google AutoML로 테스트  기본적인 성능 확인
  - Scikit-learn 내 회귀 모델 중 고성능 모델 몇가지를 샘플로 성능 확인
  - 테스트 시 높은 R2 Score를 기록한 모델을 5가지 선별해 파라미터 튜닝 후 성능 확인
    - 선별기준 
      1. R2 Score 
      2. 기반 알고리즘이 유형이 유사하지 않을 것 (Stacking 기법 사용을 위해)
  - 서울시 전체를 모델에 학습 시켰을 때 금액 편차가 심한 것을 확인하고 자치구를 기준으로 데이터를 구분해 모델 학습 진행
  - 5가지 모델을 Stacking 기법을 적용해 최종 모델로 만들고 WEB 서비스에 탑재
