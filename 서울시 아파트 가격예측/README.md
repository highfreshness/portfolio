...
- 유저에게 아파트 정보를 입력받아 현재 아파트 가격 예측을 해주는 웹 서비스 구현
...
- 전처리
  ...
  - 불필요 Feature 삭제
    ...
    - 예측과 관련 없는 Feature (실거래가아이디, 업무구분, 물건번호)
      - 예측에 불필요하다고 판단되어 삭제
    - 다른 Feature로 대체 가능한 Feature (지번코드, 시군구코드, … 관리구분코드, 건물주용도코드)
      - 기존에 있는 명칭으로 대체
    - 예측 대상이 아파트이기 때문에 삭제 (아파트는 대지권면적이 없음)
...
- 모델 / 분석
  ...
  - 전처리 완료된 데이터를 Google AutoML을 사용해 성능 확인
  - Scikit-learn 내 회귀 모델 중 일부를 선택해 기본 HyperParameter로 모델 학습하며 성능지표 확인
  - 테스트 시 높은 R2 Score를 기록한 모델을 5가지 선별해 GridSearch로 HPO 진행
    - 선별기준 
      1. R2 Score 
      2. 예측 기반 알고리즘이 중복되지 않을 것
  - 서울시 구 별로 실거래 데이터를 나누어 모델 학습 진행
  - HPO까지 마친 회귀 모델 4~5개를 Stacking 기법을 적용해 최종 모델 학습
...
- 모델 학습 시 R2 Score 값이 `-`가 되는 경우

  - R2 Score 의 계산방식에서는 -가 보통은 나올 수 없기 때문에 HyperParameter 조정을 통해 정상화 

- 다양한 아파트 이름 속에서 특정 브랜드 아파트만 찾아내는 조건

  - pandas 기능 내에 contains 함수를 사용하면 해당 Feature 내 값에 해당하는 값이 있으면 True를 반환해주는데 contains에는

    str만 사용 가능하기 때문에 여러가지 조건을 한번에 찾을 수가 없어 ```brand = '|'.join(brands)``` 처럼 join을 통해 여러 조건에 해당하는 list 데이터들을 str로 변환해 찾고 싶은 데이터를 손 쉽게 찾아낼 수 있었다.
...
- 구 별로 나누어진 모델을 동 단위까지 세분화 후 데이터 추가
...
- 반복적인 모델 작업 시 반복문 활용 방법

- 구 별로 RMSE 값이 차이가 나는 이유

  - 형성된 부동산의 시세에 따라 같은 R2 Score 값을 가지고 있어도 RMSE 값이 차이날 수 있다. 

    (부동산 시세 ↑ = RMSE↑)

- Stacking 활용 시 동일한 예측 기반 모델보다는 다양한 예측 기반 모델을 섞는 것이 성능향상에 도움을 줄 수 있다.