...
- 2021년 11월 02일 ~ 11월 12일 까지(9일)

------

## 📌프로젝트 참여 인원

- 데이터 EDA, 전처리, 예측모델 구현 - 3명 ( 해당 파트 담당)
- Django 웹 서비스 구현 - 3명 
...
- 데이터 소개
  ...
  - 2016년 ~ 2020년 서울특별시 부동산 실거래가 정보 (서울시 열린데이터 광장)

  - 2021년 서울특별시 부동산 실거래가 정보 (국토교통부 실거래가 공개시스템)
...
- 전처리 과정
  ...
  - 불필요한 Feature 삭제
    ...
    - 예측과 관련 없는 Feature (실거래가아이디, 업무구분, 물건번호 등) 삭제
    - 다른 Feature로 대체 가능한 Feature (지번코드, 시군구코드, … 관리구분코드, 건물주용도코드) 기존 명칭 대체
    - 대지권 면적 Feature 삭제 (아파트는 대지권면적이 없음)
    
  - 2021년도 실거래 데이터 매칭 작업

    - 기존 Feature를 2016~2021 실거래 데이터에 맞게 가공, 단위 변경 진행 (시군구, 거래금액 등)
  - Feature 구성이 기존과 다르거나 매칭이 힘든 부분은 삭제 (번지,본번,거래유형,…도로명,중개사소재지 등)
...
- 모델 학습 시 R2 Score 값이 `-` 로 나타나는 경우가 있었는데 R2 Score 의 계산방식에서는 -가 나올 수 없는 것이 정상이기 때문에 잘못된 값으로 판단하고 Hyper-Parameter 조정을 통해 제대로 된 수치를 구현해 정상화

  

- 다양한 아파트 이름 속에서 특정 브랜드 아파트 이름을 포함한 아파트만 찾아 별도의 Feature를 생성해 가산점을 주는데 pandas 기능 내에 contains 함수를 사용하면 해당 Feature 내 값에 해당하는 값이 있으면 True를 반환 주는 기능을 찾아 해당 기능을 통해 새로운 Feature를 생성

  - contains 기능은 str 형태만 사용 가능하기 때문에 여러가지 조건에 해당하는 값들을 한번에 찾을 수가 없어 ```brand = '|'.join(brands)``` 처럼 join을 통해 여러 조건에 해당하는 list 데이터들을 str로 변환해 해결

    
  
- 서울시 전체를 대상으로 모델을 학습시킨 경우 MSE 값과 R2 Score가 예상보다 만족스럽지 않아 자료를 다시 검토해 본 결과 구별로 아파트의 가격이 천차만별로 다른 양상을 보여 결국 1개의 모델에서 서울시 내 모든 지역구 별로 모델을 학습시켜 MSE 값과 R2 Score를 향상 시키는데 성공했다.
...
- 구 별로 나누어진 모델을 동 단위까지 세분화
  - 부동산 데이터가 더 추가 된다는 가정 아래 실현 가능할 것으로 추측
...
- 구 별로 RMSE 값이 차이가 나는 이유처럼 데이터에 대해 다양한 시각으로 접근을 해야 문제 해결이 수월해질 수 있다.
  ...
  - 형성된 부동산의 시세에 따라 같은 R2 Score 수치를 가지고 있어도 RMSE 값이 차이날 수 있다는 사실을 확인했다. (부동산 시세 ↑ = RMSE↑)

- 모델을 여러개를 사용해 성능 향상을 올려주는 Stacking 같은 기법을 활용 시 다양한 예측 기반 모델을 섞어 주는 것이 성능 향상에 도움을 줄 수 있다.
- 머신러닝/딥러닝 모델의 성능은 데이터의 양이 가장 높은 영향을 미치고 그외 수단(HPO.. 정규화 등)에 대해서는 데이터의 양이 늘어나는 것보다 극적인 효과는 없는 것으로 보여진다.